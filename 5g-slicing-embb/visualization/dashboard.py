"""
Visualization Dashboard for eMBB Network Slicing Project.
Generates all analysis plots:
  1. Traffic time-series with daily patterns
  2. LSTM prediction overlay
  3. Controller decision timeline
  4. Baseline comparison charts
  5. Slice resource utilization
...
def plot_traffic_timeseries():
    """Plot 1: Raw traffic time-series showing eMBB patterns."""
    df = pd.read_csv(os.path.join(DATA_DIR, 'embb_traffic_timeseries.csv'))
    df['timestamp'] = pd.to_datetime(df['timestamp'])

    fig, axes = plt.subplots(3, 1, figsize=(16, 10), sharex=True)
    fig.suptitle('eMBB Traffic Time-Series Analysis', fontsize=14, fontweight='bold')

    # Throughput
    axes[0].plot(df['timestamp'], df['throughput_mbps'], color='#3498db', linewidth=0.5, alpha=0.8)
    axes[0].fill_between(df['timestamp'], df['throughput_mbps'], alpha=0.2, color='#3498db')
    axes[0].set_ylabel('Throughput (Mbps)')
    axes[0].set_title('Throughput Over Time')
    axes[0].grid(True, alpha=0.3)
    axes[0].axhline(y=70, color='red', linestyle='--', alpha=0.5, label='High threshold (70 Mbps)')
    axes[0].axhline(y=30, color='green', linestyle='--', alpha=0.5, label='Low threshold (30 Mbps)')
    axes[0].legend(loc='upper right')

    # Packet rate
    axes[1].plot(df['timestamp'], df['packet_rate'], color='#e74c3c', linewidth=0.5, alpha=0.8)
    axes[1].set_ylabel('Packet Rate (pps)')
    axes[1].set_title('Packet Rate Over Time')
    axes[1].grid(True, alpha=0.3)

    # Active UEs
    axes[2].plot(df['timestamp'], df['active_ues'], color='#2ecc71', linewidth=0.8, alpha=0.8)
    axes[2].set_ylabel('Active UEs')
    axes[2].set_title('Active UE Count Over Time')
...
def plot_controller_decisions():
    """Plot 3: Controller decision timeline."""
    ...
    fig.suptitle('Zero-Touch Controller Decision Timeline', fontsize=14, fontweight='bold')

    # Traffic with decision overlay
    axes[0].plot(df['throughput_mbps'].values, color='#3498db', linewidth=0.5, alpha=0.7,
                 label='Actual throughput')
    axes[0].axhline(y=70, color='red', linestyle='--', alpha=0.4, label='High threshold')
    axes[0].axhline(y=30, color='green', linestyle='--', alpha=0.4, label='Low threshold')

    # Mark expand/contract decisions
...
def plot_baseline_comparison():
    """Plot 4: Baseline comparison bar charts."""
    ...
    metrics = ['sla_violation_rate_pct', 'utilization_pct', 'packet_loss_pct', 'resource_waste_pct']
    titles = ['SLA Violation Rate (%)', 'Resource Utilization (%)',
              'Packet Loss (%)', 'Resource Waste (%)']

    fig, axes = plt.subplots(2, 2, figsize=(14, 10))
    fig.suptitle('Performance Comparison: Static vs Reactive vs Proactive', fontsize=14, fontweight='bold')
    ...
    for idx, (metric, title) in enumerate(zip(metrics, titles)):
        ...
        ax.set_title(title)
        ax.set_ylabel(title.split('(')[-1].replace(')', '').strip() or '%')
...
def plot_slice_utilization():
    """Plot 5: Slice resource utilization heatmap-style."""
    df = pd.read_csv(os.path.join(DATA_DIR, 'embb_traffic_timeseries.csv'))
    throughput = df['throughput_mbps'].values

    # Reshape into 2D (hours x samples_per_hour)
    samples_per_hour = 720  # 3600/5
    n_hours = len(throughput) // samples_per_hour
    data = throughput[:n_hours * samples_per_hour].reshape(n_hours, samples_per_hour)

    fig, ax = plt.subplots(figsize=(14, 4))
    im = ax.imshow(data, aspect='auto', cmap='RdYlGn_r', vmin=0, vmax=100)
    ax.set_xlabel('Time within hour (samples)')
    ax.set_ylabel('Hour')
    ax.set_title('eMBB Slice Throughput Heatmap (Mbps)', fontsize=14, fontweight='bold')
    plt.colorbar(im, ax=ax, label='Throughput (Mbps)')

    plt.tight_layout()
    path = os.path.join(VIZ_DIR, 'slice_utilization_heatmap.png')
    plt.savefig(path, dpi=150)
    plt.close()
    print(f"✓ Utilization heatmap → {path}")


def main():
    print("=" * 60)
    print("  eMBB Network Slicing — Visualization Dashboard")
    print("=" * 60)
    print()

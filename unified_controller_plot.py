...
def _style_ax(ax, title: str, ylabel: str):
    """Apply the dark-theme style from con_test.py to an Axes object."""
...
def _plot_urllc_summary(decisions: list, out_dir: str) -> str:
    """
    One-shot URLLC summary dashboard from the full decisions list.
    Panels: latency trigger | 5QI over time | ARP over time | action counts.
    """
    try:
        import matplotlib
        matplotlib.use('Agg')
    ...
    fig.suptitle(
        f'URLLC Slice — Full Simulation Summary  |  {len(decisions)} decisions'
    ...
    _style_ax(ax1, '📡 Trigger Latency (ms)', 'ms')
    ...
    out = os.path.join(out_dir, 'summary_urllc.png')
...
def _plot_mmtc_summary(decisions: list, out_dir: str) -> str:
    """
    One-shot mMTC summary dashboard.
    Panels: packet-rate trigger | capacity_pps | BW DL | action counts.
    """
    try:
        import matplotlib
        matplotlib.use('Agg')
    ...
    fig = plt.figure(figsize=(16, 10), facecolor='#1a1a2e')
    fig.suptitle(
        f'mMTC Slice — Full Simulation Summary  |  {len(decisions)} decisions'
        f'  |  {ts[0][0:10]} → {ts[-1][0:10]}',
        fontsize=13, color='white', fontweight='bold', y=0.98
    )
    gs = gridspec.GridSpec(2, 2, left=0.07, right=0.96, hspace=0.40, wspace=0.28)
    ...
    _style_ax(ax1, '📡 Trigger Packet Rate (pps)', 'pps')
    ...
    out = os.path.join(out_dir, 'summary_mmtc.png')
    plt.savefig(out, dpi=130, bbox_inches='tight',
                facecolor=fig.get_facecolor())
    plt.close()
    return out
...
def _plot_embb_summary(decisions: list, out_dir: str) -> str:
    """
    One-shot eMBB summary dashboard.
    Panels: throughput trigger | slice BW DL/UL | per-subscriber BW | action counts.
    """
    try:
        import matplotlib
        matplotlib.use('Agg')
    ...
    fig = plt.figure(figsize=(16, 10), facecolor='#1a1a2e')
    fig.suptitle(
        f'eMBB Slice — Full Simulation Summary  |  {len(decisions)} decisions'
        f'  |  {ts[0][0:10]} → {ts[-1][0:10]}',
        fontsize=13, color='white', fontweight='bold', y=0.98
    )
    gs = gridspec.GridSpec(2, 2, left=0.07, right=0.96, hspace=0.40, wspace=0.28)
    ...
    ax1.scatter(x, trig, c=point_colors, s=12, zorder=4)
    ax1.set_xticks(x); ax1.set_xticklabels(xlbls)
    _style_ax(ax1, '📡 Trigger Throughput (Mbps)', 'Mbps')
    ...
    out = os.path.join(out_dir, 'summary_embb.png')
    plt.savefig(out, dpi=130, bbox_inches='tight',
                facecolor=fig.get_facecolor())
    plt.close()
    return out
...
def generate_slice_visualizations(out_dir: Optional[str] = None):
    """
    Load the three decision JSON files written by the controller processes
    and produce one summary PNG per slice.

    Called automatically by main() once all processes have joined.
    Can also be called stand-alone::

        from unified_controller import generate_slice_visualizations
        generate_slice_visualizations()
    """
    if out_dir is None:
        out_dir = os.path.join(ROOT_DIR, 'unified_visualizations')
    os.makedirs(out_dir, exist_ok=True)

    mapping = [
        ('URLLC', os.path.join(ROOT_DIR, 'unified_decisions_urllc.json'),
         _plot_urllc_summary),
        ('mMTC',  os.path.join(ROOT_DIR, 'unified_decisions_mmtc.json'),
         _plot_mmtc_summary),
        ('eMBB',  os.path.join(ROOT_DIR, 'unified_decisions_embb.json'),
         _plot_embb_summary),
    ]

    print('\n  Generating slice summary visualizations…')
    saved = []
